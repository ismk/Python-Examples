...
# open a txt file
    def filesystem_access(self):
        filename = open(
            askopenfilename(filetypes=[("allfiles", "*"), ("textfiles", "*.txt")]))
...
# save to a text file
    ...
    def savetofilesystem(self):
        filename = open(
            askopenfilename(filetypes=[("textfiles - will overwrite", "*.txt")]), "w")
    ...
    def create_widgets(self):
        ...
        self.submit_button = Button(
            self, text="Submit", command=self.reveal)
        self.submit_button.grid(row=2, column=0, sticky=W)

        self.delete_button = Button(
            self, text="Clear", command=self.delete)
...
# output when button is pressed
    def reveal(self):
        ...
        for z in range(0, len(phonestring)):
            if (phonestring[z] in phone) == True:
